...
public:
    ...
    const MLP_Layer * GetLayerNetwork() const {return this->layerNetwork;}
    ...
    friend ifstream& operator>>(ifstream& is, MLP_Network& mlp);
	//! fonction de sauvegarde du réseau dans un fichier
    friend ofstream& operator<<(ofstream& os, const MLP_Network& mlp);
};
#endif
