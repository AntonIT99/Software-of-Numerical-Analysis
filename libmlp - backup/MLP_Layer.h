...
public:
    ...
    //! fonction qui retroune le nombre de neurone de la couche courante
    ...
    inline FLOAT_TYPE ActivationFunction(INTERNAL_FLOAT_TYPE value)
        {
	...
	//! calcul du gradient de la fonction d'activation
    inline FLOAT_TYPE DerivativeActivation(INTERNAL_FLOAT_TYPE value)
        {
        if (Activation_function == 'S')  { return (1.0 - value) * value;              }    // Sigmoid
