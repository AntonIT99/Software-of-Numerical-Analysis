...
*/
class MLP_Network_SGD_LR
{
protected:
    //! Tableau des différentes couches (layer)
    MLP_Layer_SGD_LR *layerNetwork;
...
public:
    ...
    MLP_Network_SGD_LR( char activation='L'):
    ...
    virtual ~MLP_Network_SGD_LR(){ Delete(); }
    ...
    const MLP_Layer_SGD_LR * GetLayerNetwork() const {return this->layerNetwork;}
    ...
    friend ifstream& operator>>(ifstream& is, MLP_Network_SGD_LR& mlp);
    //! fonction de sauvegarde du réseau dans un fichier
    friend ofstream& operator<<(ofstream& os, const MLP_Network_SGD_LR& mlp);
