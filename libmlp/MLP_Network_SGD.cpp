...
void MLP_Network_SGD::Allocate(int nInputUnit,   int nHiddenUnit, int nOutputUnit, int nHiddenLayer,
                           int nTrainingSet)
{
    ...
    layerNetwork = new MLP_Layer_SGD[nHiddenLayer+1];
...
void MLP_Network_SGD::ForwardPropagateNetwork(float* inputNetwork)
{
...
void MLP_Network_SGD::BackwardPropagateNetwork(float* desiredOutput)
{
...
void MLP_Network_SGD::UpdateWeight(float learningRate)   // update weight according gradient and standart gradient descent
{
...
float MLP_Network_SGD::CostFunction(float* desiredOutput)
{
...
float MLP_Network_SGD::CalculateResult(float* desiredOutput)
{
    int maxIdx = 0;
...
ifstream& operator>>(ifstream& is, MLP_Network_SGD& mlp)
{
...
ofstream& operator<<(ofstream& os, const MLP_Network_SGD& mlp)
{
