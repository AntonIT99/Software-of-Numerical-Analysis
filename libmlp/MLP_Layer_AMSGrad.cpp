...
void MLP_Layer_AMSGrad::Allocate(int previous_num, int current_num)
{
    ...
    SW_hat   = new INTERNAL_FLOAT_TYPE[nPreviousNeurons * nCurrentNeurons];
    Sb       = new INTERNAL_FLOAT_TYPE[nCurrentNeurons];
    Sb_hat   = new INTERNAL_FLOAT_TYPE[nCurrentNeurons];
    ...
    for (int j = 0; j < nCurrentNeurons; j++)
        ...
        for (int i = 0; i < nPreviousNeurons; i++)
            ...
            SW_hat[j*nPreviousNeurons+i] = 0.0;
            }
        Mb[j]  = 0.0;
        Sb[j]  = 0.0;
        Sb_hat[j]  = 0.0;
...
void MLP_Layer_AMSGrad::Delete()
{
 // ADAM
    ...
    if( SW_hat      != nullptr) { delete [] SW_hat;    SW_hat = nullptr; }
    if( Sb          != nullptr) { delete [] Sb;            Sb = nullptr; }
    if( Sb_hat      != nullptr) { delete [] Sb_hat;    Sb_hat = nullptr; }
    if( SW_next     != nullptr) { delete [] SW_next;  SW_next = nullptr; }
    if( Sb_next     != nullptr) { delete [] Sb_next;  Sb_next = nullptr; }
}

void MLP_Layer_AMSGrad::UpdateWeight(FLOAT_TYPE learningRate)
{
...
//////////////// W
   ...
   for (int j = 0; j < nCurrentNeurons; j++)
   {
       for (int i = 0; i < nPreviousNeurons; i++)
       {
           ...
           SW_hat[j*nPreviousNeurons + i] = (SW_next[j*nPreviousNeurons + i] > SW_hat[j*nPreviousNeurons + i])? SW_next[j*nPreviousNeurons + i] : SW_hat[j*nPreviousNeurons + i];

           W[j*nPreviousNeurons + i] -=  alpha_T * MW_next[j*nPreviousNeurons + i] / (sqrt(SW_hat[j*nPreviousNeurons + i]) + Epsilon);
       }
    }
...
//////////////// b
   ...
   for (int j = 0; j < nCurrentNeurons; j++)
   {
       ...
       Sb_hat[j] = (Sb_next[j]>Sb_hat[j])? Sb_next[j] : Sb_hat[j];

       b[j] -=  alpha_T * Mb_next[j] / (sqrt(Sb_hat[j]) + Epsilon);
    }
