...
void MLP_Network_AMSGrad::Allocate(int nInputUnit,   int nHiddenUnit, int nOutputUnit, int nHiddenLayer,
                           int nTrainingSet)
{
    ...
    layerNetwork = new MLP_Layer_AMSGrad[nHiddenLayer+1];
...
void MLP_Network_AMSGrad::Delete()
{
    for (int i = 0; i < nHiddenLayer+1; i++)
    {
...
void MLP_Network_AMSGrad::ForwardPropagateNetwork(float* inputNetwork)
{
...
void MLP_Network_AMSGrad::BackwardPropagateNetwork(float* desiredOutput)
{
...
void MLP_Network_AMSGrad::UpdateWeight(float learningRate)   // update weight according gradient and standart gradient descent
{
...
float MLP_Network_AMSGrad::CostFunction(float* desiredOutput)
{
...
float MLP_Network_AMSGrad::CalculateResult(float* desiredOutput)
{
    int maxIdx = 0;
...
ifstream& operator>>(ifstream& is, MLP_Network_AMSGrad& mlp)
{
...
ofstream& operator<<(ofstream& os, const MLP_Network_AMSGrad& mlp)
{
