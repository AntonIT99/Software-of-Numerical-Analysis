...
#endif
    INTERNAL_FLOAT_TYPE derivate;
    for (int k = 0; k < nCurrentNeurons; k++)
        {
//        derivate = DerivativeActivation(outputLayer[k]);
//        derivate = DerivativeActivation(preActivationLayer[k]);   //!preActivationLayer
//        Delta[k] =  derivate * (desiredValues[k] - outputLayer[k]);
        Delta[k] = (desiredValues[k] - outputLayer[k]);
        }
...
//        Delta[j] =  DerivativeActivation(preActivationLayer[j])* previous_sum; //!preActivationLayer
        }
